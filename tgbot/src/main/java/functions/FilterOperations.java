...
public class FilterOperations {
    ...
    @AppBotCommand(name= "contrastRGB", description = "contrastRGB filter", showInKeyboard = true)
    public static float[] contrastRGB(float[] rgb){
        final float mean = (rgb[0] + rgb[1] + rgb[2])/3;
        final float max = Math.max(rgb[0],Math.max(rgb[1],rgb[2]));
        final float min = Math.min(rgb[0],Math.min(rgb[1],rgb[2]));


        rgb[0] = Math.round(rgb[0]* 0.2126);
        rgb[1] = Math.round(rgb[1]* 0.7152);
        rgb[2] = Math.round(rgb[2]* 0.0722);


//        rgb[0] = (mean>rgb[0])?mean-rgb[0]:rgb[0]-mean;
//        rgb[1] = (mean>rgb[1])?mean-rgb[1]:rgb[1]-mean;
//        rgb[2] = (mean>rgb[2])?mean-rgb[2]:rgb[2]-mean;
        return rgb;
    }
